...
.main-title { font-family: 'Playfair Display', serif; font-size: 2.4rem; color: #1a3a5c; letter-spacing: -0.5px; }
...
.alert-box { background: #fff3cd; border-left: 4px solid #ffc107; padding: 12px 16px; border-radius: 0 8px 8px 0; margin: 8px 0; }
...
.telegram-badge { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 500; }
...
with st.sidebar:
    ...
    with st.expander("⚙️ Configuration", expanded=False):
        st.markdown("""
        <small>Les alertes sont envoyées automatiquement quand une analyse détecte
        des <b>signaux critiques</b> (risques élevés, opportunités majeures).</small>
        """, unsafe_allow_html=True)
        st.markdown("")
        st.markdown("**Signaux déclencheurs :**")
        st.markdown("""
        <small>
        🔴 Risque CRITIQUE détecté<br>
        • Coup d'état / instabilité<br>
        • Crise diplomatique<br>
        • Dette souveraine / défaut<br>
        • Radicalisation / conflit<br>
        • Opportunité majeure (3+ signaux)
        </small>
        """, unsafe_allow_html=True)
        st.markdown("")
        if st.button("🧪 Tester la connexion", use_container_width=True):
            ...
            if result["success"]:
                ...
                st.markdown("""
                <small>Vérifiez dans .env :<br>
                <code>TELEGRAM_BOT_TOKEN=xxx</code><br>
                <code>TELEGRAM_CHAT_ID=xxx</code></small>
                """, unsafe_allow_html=True)

    st.markdown("---")
    st.markdown("**📋 Historique**")
    for item in reversed(st.session_state.history[-5:]):
        tg_icon = "📨" if item.get("telegram", {}).get("sent") else "💬"
        st.markdown(f"""<div class="history-item">
            {tg_icon} <b>{item['timestamp']}</b><br>
            {item['query'][:55]}...
...
with col1:
    ...
    st.markdown('<p class="subtitle">Analyses politiques · Économiques · Humaines · Opportunités d\'investissement</p>', unsafe_allow_html=True)
with col2:
...
query = st.text_area("Question", value=default_query,
    placeholder="Ex: Quelles sont les meilleures opportunités dans le secteur agro-industriel ?",
...
if run_analysis and query.strip():
    with st.spinner("⏳ Analyse en cours..."):
        params = {"angle": angle, "influences": influences,
                  "horizon": horizon, "secteur": secteur}
        result, telegram_result = st.session_state.analyst.analyze(query, params)
        st.session_state.last_result = result
        st.session_state.last_telegram = telegram_result
        st.session_state.history.append({
            "timestamp": datetime.now().strftime("%H:%M"),
            "query": query, "params": params,
            "result": result, "telegram": telegram_result
        })
    st.rerun()
    ...
    if telegram:
        if telegram.get("sent"):
            ...
            st.markdown(f"""<div class="alert-critical">
                📨 <b>Alerte Telegram envoyée</b> — Signaux critiques détectés : {kw_list}
            </div>""", unsafe_allow_html=True)
        elif telegram.get("critical"):
            st.markdown(f"""<div class="alert-box">
                ⚠️ <b>Analyse critique</b> — {telegram.get('status', '')}
            </div>""", unsafe_allow_html=True)
        else:
